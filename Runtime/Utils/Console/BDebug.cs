...
namespace BigasTools{
    public static class BDebug
    {
        ...
        public static void LogWarning(object message, string title = "Log", Color? color = null, bool isCache = false){
            if(cache.Contains(message) && isCache)return;
            var c = color == null ? Color.yellow : color.Value;
            if(isCache)cache.Add(message);
            Debug.LogWarning(string.Format("<color=#{0:X2}{1:X2}{2:X2}><b>{3}</b> || </color>{4}", (byte)(c.r * 255f), (byte)(c.g * 255f), (byte)(c.b * 255f), title, message)); 
        }
